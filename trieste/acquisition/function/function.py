...
def entropy_search(at, query_points, models: dict, datasets: dict):
    """
    An Entropy based Acquisition Function:
    H[g(x^*|D)] - E_f(x^**) H[g(x^*|D,{x^**, f(x^**)})]
    The E_f(x^**) is approximated by posterior mean (means without sample)
    :param query_points: x^*
    :param: at: x^**
    :param: query_points
    """
    # print(at)
    tf.debugging.assert_shapes([(at, (1, "D"))])
    tf.debugging.assert_shapes([(query_points, (1, "D"))])
    from trieste.acquisition.rule import OBJECTIVE

    g_var = models[OBJECTIVE].predict(query_points)[1]
    # 1st term: assume independent at different obj
    first_term = 0.5 * tf.math.log(tf.reduce_prod(g_var, axis=1))

    # calculate 2nd term:
    predict_f = [
        models[OBJECTIVE]._models[model_idx]._model.predict_f(at)
        for model_idx in range(len(models[OBJECTIVE]._models))
    ]
    predict_f_mean, _ = (tf.concat(moment, 1) for moment in zip(*predict_f))
    # 2nd term:
    # TODO: Only support batch size=1
    # pending_models = {tag: copy(m) for tag, m in models.items()}
    # for model_tag, idx in zip(models, range(len(pending_models))):
    #     pending_models[model_tag].update(
    #         datasets[model_tag]
    #         + Dataset(query_points=at, observations=tf.gather(predict_f_mean, [1], axis=1))
    #     )
    pending_models = copy.deepcopy(models[OBJECTIVE])
    pending_models.update(
        datasets[OBJECTIVE] + Dataset(query_points=at, observations=predict_f_mean)
    )

    _new_pred_g_var = pending_models.predict(query_points)[1]
    second_term = 0.5 * tf.math.log(tf.reduce_prod(_new_pred_g_var, axis=1))
    return first_term - second_term


def stanard_deviation(at: TensorType, models: dict):
    from trieste.acquisition.rule import OBJECTIVE

    return tf.sqrt(models[OBJECTIVE].predict(at)[-1])


def probability_of_standard_deviation(at: TensorType, models: dict, base_prob, bias: TensorType):
    from trieste.acquisition.rule import OBJECTIVE

    # this reduce prod is used for uniform distribution as it regard multple input as batch
    prob = base_prob.prob(tf.squeeze(at) - bias)
    if tf.rank(prob) > 0: # only used for uniform input uncertainty
        prob = tf.reduce_prod(prob, axis=-1)
    return prob * tf.sqrt(
        models[OBJECTIVE].predict(at)[-1]
    )
