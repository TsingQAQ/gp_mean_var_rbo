...
from .. import types
from ..acquisition.function.robust import MO_MVA_BO
...
class EfficientGlobalOptimization(AcquisitionRule[TensorType, SP_contra]):
    ...
    def __init__(
        ...
        **kwargs_for_optimizer,
        ...
        self._kwargs_for_optimizer = kwargs_for_optimizer
    ...
    def acquire(
        ...
        models: Mapping[str, ProbabilisticModel],
        datasets: Optional[Mapping[str, Dataset]] = None,
        return_acquisition_builder: bool = False,
    ) -> Union[TensorType, tuple]:
        """
        ...
        :param return_acquisition_builder
        ...
        points = self._optimizer(
            search_space, self._acquisition_function, **self._kwargs_for_optimizer
        )
        ...
        if not return_acquisition_builder:
            return points
        else:
            return points, self._builder
...
class ActiveLearningAcquisitionRule((AcquisitionRule[TensorType, SP_contra])):
    """Implements the Active Learning Rule."""
    ...
    def __init__(
        ...
        optimizer: AcquisitionOptimizer[SP_contra] | None = optimize_discrete,
        num_query_points: int = 1,
        **kwargs_for_optimizer,
    ):
        """
        ...
        :param num_query_points: The number of points to acquire.
        """
        # assert isinstance(builder, MO_MVA_BO)
        if num_query_points <= 0:
            raise ValueError(
                f"Number of query points must be greater than 0, got {num_query_points}"
            )
    ...
    def acquire(
        ...
        pool = search_space.discretize_v2(self._builder.tau)  # [N, D]
        # search and eliminate already visited point
        not_queried_mask = ~tf.reduce_any(
            tf.reduce_all(
                tf.equal(tf.expand_dims(pool.points, -2), datasets["OBJECTIVE"].query_points), -1
            ),
            -1,
        )
        candidate_search_space = DiscreteSearchSpace(points=pool.points[not_queried_mask])
        # ==========================================
        # TODO: Figuring out why this looks like this
        if self._acquisition_function is None:
            (
                self._acquisition_function,
                potential_optimal_search_space,
            ) = self._builder.prepare_acquisition_function(
                models, datasets=datasets, search_candidates=candidate_search_space.points
            )
        else:
            (
                self._acquisition_function,
                ...
                models,
                datasets=datasets,
                search_candidates=candidate_search_space.points,
            )

        # TODO:
        points = self._optimizer(
            potential_optimal_search_space, self._acquisition_function, **self._kwargs_for_optimizer
        )
...
class Random(AcquisitionRule[TensorType, SP_contra]):
    """
    A rule used more for comparison
    """
    ...
    def acquire(
        ...
        points = search_space.sample(self._num_query_points)
        if not return_acquisition_builder:
            return points
        else:
            return points, self._builder
