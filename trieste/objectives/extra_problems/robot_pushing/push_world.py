...
class guiWorld:
    ...
    def draw(self, bodies, bg_color=(64, 64, 64, 0)):
        ...
        def my_draw_polygon(polygon, body, fixture):
            vertices = [
                (self.screen_origin + body.transform * v) * self.PPM for v in polygon.vertices
            ]
        ...
        def my_draw_circle(circle, body, fixture):
            ...
            if body.userData == "hand":
                color = (174, 136, 218, 0)
            pygame.draw.circle(
                self.screen, color, [int(x) for x in position], int(circle.radius * self.PPM)
            )
...
class end_effector:
    ...
    def get_state(self, verbose=False):
        ...
        if verbose:
            ...
            print(
                f"position, velocity: {(',').join(print_state[:3])}, {(',').join(print_state[3:])} "
            )
...
def simu_push(world, thing, robot, base, simulation_steps):
    ...
    for t in range(simulation_steps + 100):
        ...
        ostate = (
            list(thing.position)
            + [thing.angle]
            + list(thing.linearVelocity)
            + [thing.angularVelocity]
        )
...
def simu_push2(world, thing, robot, base, xvel, yvel, simulation_steps):
    ...
    rvel = b2Vec2(
        desired_vel[0] + np.random.normal(0, 0.1), desired_vel[1] + np.random.normal(0, 0.1)
    )
...
def make_1thing(
    base, b2world_interface, thing_shape, thing_size, thing_friction, thing_density, obj_loc
):
    world = b2world_interface.world
...
def simu_push_2robot2thing(
    ...
    desired_vel = np.array([xvel, yvel])
    rvel = b2Vec2(
        desired_vel[0] + np.random.normal(0, 0.01), desired_vel[1] + np.random.normal(0, 0.01)
    )

    desired_vel2 = np.array([xvel2, yvel2])
    rvel2 = b2Vec2(
        desired_vel2[0] + np.random.normal(0, 0.01), desired_vel2[1] + np.random.normal(0, 0.01)
    )
