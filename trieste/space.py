...
class Box(SearchSpace):
    ...
    def discretize_v2(self, tau: float):
        # here we force each dim's distance is the same for a easier coding
        assert tf.reduce_all(self.lower == 0.0)
        assert tf.reduce_all(self.upper == 1.0)
        if self.dimension == 1:  # input dim == 1, doesn't support tf.split
            return DiscreteSearchSpace(
                tf.cast(tf.linspace([0], [1], int(1 / tau)), dtype=tf.float64)
            )
        else:
            return DiscreteSearchSpace(
                points=tf.squeeze(
                    tf.convert_to_tensor(
                        list(
                            product(
                                *tf.split(
                                    tf.linspace(
                                        tf.zeros(self.dimension),
                                        tf.ones(self.dimension),
                                        int(1 / tau),
                                    ),
