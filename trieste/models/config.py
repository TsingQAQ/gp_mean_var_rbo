...
class ModelConfig:
    ...
    def _check_model_type(self) -> None:
        if type(self.model) in ModelRegistry.get_registered_models():
            ...
            print(f"Not Officially Supported Type {type(self.model)} feeded in ")
            print("Force allow this model pass, Careful!!!")
            return
    ...
    def create_robust_model_interface(self) -> TrainableProbabilisticModel:
        # from .model_interfaces import GPR, robustGaussianProcessRegression
        from .gpflow.models import GPR, robustGaussianProcessRegression
        ...
        if isinstance(self.model, GPR):
            return robustGaussianProcessRegression(self.model, optimizer)
...
def create_robust_model(config: ModelSpec) -> TrainableProbabilisticModel:
    if isinstance(config, ModelConfig):
        return config.create_robust_model_interface()
        # return ModelConfig(**config).create_robust_model_interface()
    elif isinstance(config, TrainableProbabilisticModel):
        return config
    raise NotImplementedError("Unknown format passed to create a TrainableProbabilisticModel.")
