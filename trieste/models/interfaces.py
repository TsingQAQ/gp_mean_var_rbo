...
class RModelStack(ModelStack):
    """
    This is only useable for GPR with kernel MC
    """

    def __init__(self, *args, seed_sampler=None, aux_sampler=None, kernel_MC_num: int = 512):
        super().__init__(*args)
        self.seed_sampler = seed_sampler
        self.aux_sampler = aux_sampler
        self.kernel_MC_num = kernel_MC_num

    def update(self, dataset: Dataset, resample: bool = False) -> None:
        """
        Update all the wrapped models on their corresponding data. The data for each model is
        extracted by splitting the observations in ``dataset`` along the event axis according to the
        event sizes specified at :meth:`__init__`.
        ...
        if resample:
            # Resample
            seed_sample = self.seed_sampler.sample(self.kernel_MC_num)
            aux_sample = self.aux_sampler.sample(self.kernel_MC_num)
            for model, obs in zip(self._models, observations):
                model.update(Dataset(dataset.query_points, obs), seed_sample, aux_sample)
        else:
            for model, obs in zip(self._models, observations):
                model.update(Dataset(dataset.query_points, obs))
