...
from docs.exp.FF_Variance.robust_bayesian_optimization_exp.scalar_mean_var_exp.cfg import pb_cfgs
...
from trieste.acquisition.function.robust import MT_MVA_BO, US, FF_SMV_qEI, Random_Acq
...
def main(**kwargs):
    if kwargs != {}:
        ...
        # kwargs_for_obj = kwargs["kw_obj"] if "kw_obj" in kwargs else {}
        ...
        scalarization_alpha = kwargs["sc_alpha"] if "sc_alpha" in kwargs else ValueError
    else:  # use argparse
        ...
        parser.add_argument("-s_a", "--sc_alpha", type=float)
        ...
        scalarization_alpha = _args.sc_alpha
    ...
    if acq_name == "FF-SMV-qEI":
        acq = FF_SMV_qEI
    elif acq_name == "MT-MVA-BO":
        acq = MT_MVA_BO
    elif acq_name == "US":
        acq = US
    ...
    for rec_key, recommender in pb_cfg["recommender"].items():
        if rec_key == "In_Sample":
            pb_cfg["recommender"][rec_key] = partial(
                recommender, scalarization_alpha=scalarization_alpha
            )
    ...
    for metric_key, metric_func in pb_cfg["post_metrics"].items():
        if metric_key == "Regret":
            metrics[metric_key] = partial(
                ...
                scalarize_alpha=scalarization_alpha,
            )
        else:
