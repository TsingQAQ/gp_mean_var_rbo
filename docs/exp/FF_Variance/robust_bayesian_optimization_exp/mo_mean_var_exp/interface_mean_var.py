...
from docs.exp.FF_Variance.robust_bayesian_optimization_exp.mo_mean_var_exp.cfg import pb_cfgs
...
from trieste.acquisition.function.robust import MO_MVA_BO, US, FF_MV_qEHVI, Random_Acq
...
def main(**kwargs):
    if kwargs != {}:
        ...
        ref_pf_filename = kwargs["ref_pf"] if "ref_pf" in kwargs else {}
        ...
        parser.add_argument("-n_p", "--noise_param", type=float)
        parser.add_argument("-c", "--core", type=int)
        parser.add_argument("-r", "--repeat", type=int)
        parser.add_argument("-acq")
        parser.add_argument("-ref_pf", "--ref_pf_filename")
        ...
        parser.add_argument("-which_rec", type=str, default="mean_var")
    ...
    if acq_name == "FF-MV-qEHVI":
        acq = FF_MV_qEHVI
    elif acq_name == "MO-MVA-BO":
        acq = MO_MVA_BO
    elif acq_name == "US":
        acq = US
    ...
    for metric_key, metric_func in pb_cfg["post_metrics"].items():
        if metric_key == "Log_Hv":
            metrics[metric_key] = partial(
                metric_func,
                true_pf=np.loadtxt(_true_pf_dir),
                ref_point=kw_metrics["Log_Hv"]["ref_point"],
            )
        elif metric_key == "AVD":
            metrics[metric_key] = partial(
                metric_func, scaler=kw_metrics["AVD"]["scaler"], true_pf=np.loadtxt(_true_pf_dir)
            )
        else:
