...
@random_seed
@pytest.mark.parametrize(
    "training_data, kernel, noise_seed_sample, test_x, f_sample_num",
    [
        (
            (_2dx, _2dy),
            NoisyAnalyticSquaredExponential(
                lengthscales=[0.9, 0.5], noise_covar=tf.constant([0.002, 0.005], dtype=tf.float64)
            ),
            tf.cast(
                tfp.distributions.Normal(loc=[0, 0], scale=tf.sqrt([0.002, 0.005])).sample(10000),
                dtype=tf.float64,
            ),
            tf.constant([[0.5, 0.5]], dtype=tf.float64),
            1000,
        ),
    ],
)
def test_NGPRAnalyticKernel_predict_g(
    training_data: tuple,
    kernel,
    noise_seed_sample: TensorType,
    test_x: TensorType,
    f_sample_num: int,
) -> None:
    gpr = NGPRAnalyticKernel(training_data, kernel)
    opt = gpflow.optimizers.Scipy()
    _ = opt.minimize(gpr.training_loss, gpr.trainable_variables, options=dict(maxiter=10))
    pred_g_mean, pred_g_var = gpr.predict_g(test_x)
    pertubated_test_x = test_x + noise_seed_sample
    pred_f_samples = gpr.predict_f_samples(
        pertubated_test_x, f_sample_num
    )  # [MC_num, input_dist, dim]
    pred_g_samples_approx = tf.reduce_mean(pred_f_samples, axis=1)
    pred_g_mean_approx = tf.reduce_mean(pred_g_samples_approx, axis=0)
    pred_g_var_approx = tf.math.reduce_variance(pred_g_samples_approx, axis=0, keepdims=True)
    tf.debugging.assert_near(pred_g_mean, pred_g_mean_approx, rtol=0.01)
    tf.debugging.assert_near(pred_g_var, pred_g_var_approx, rtol=0.1)


@random_seed
@pytest.mark.parametrize(
    "training_data, kernel, mc_kernel, test_x, full_cov",
    [
        (
            (_2dx, _2dy),
            NoisyAnalyticSquaredExponential(
            ...
            NoisyMCSquaredExponential(
                tf.cast(
                    tfp.distributions.Normal(loc=[[0, 0]], scale=tf.sqrt([[0.002, 0.005]])).sample(
                        2000
                    ),
                    dtype=tf.float64,
                ),
            ...
            tf.constant([[0.2, 0.4], [0.5, 0.5], [0.9, 0.3]], dtype=tf.float64),
            False,
        ),
        (  # Case2: predict g with 3d input: [2, 3, 2]
            (_2dx, _2dy),
            NoisyAnalyticSquaredExponential(
                lengthscales=[0.9, 0.5], noise_covar=tf.constant([0.002, 0.005], dtype=tf.float64)
            ),
            ...
            tf.constant(
                [[[0.2, 0.4], [0.5, 0.5], [0.9, 0.3]], [[0.3, 0.3], [0.2, 0.7], [0.1, 0.5]]],
                dtype=tf.float64,
            ),
            False,
        ),
...
def test_NGPRMCKernel_predict_g(
    training_data: tuple, kernel, mc_kernel, test_x: TensorType, full_cov: bool
) -> None:
    ...
    pred_g_analytical_mean, pred_g_analytical_var = ngpr.predict_g(test_x, full_cov=full_cov)
    pred_g_mc_mean, pred_g_mc_var = nmcgpr.predict_g(test_x, full_cov=full_cov)
    tf.debugging.assert_near(pred_g_analytical_mean, pred_g_mc_mean, rtol=0.01)
    tf.debugging.assert_near(pred_g_analytical_var, pred_g_mc_var, rtol=0.02)


@random_seed
@pytest.mark.parametrize(
    "training_data, kernel, mc_kernel, test_x, f_sample_num",
    [
        (
            (_2dx, _2dy),
            NoisyAnalyticSquaredExponential(
...
def test_NGPRCKernel_predict_fullcov_approx(
    training_data: tuple,
    kernel,
    mc_kernel,
    test_x: TensorType,
    ...
    _, pred_g_analytical_cov = ngpr.predict_g(test_x, full_cov=True)
    _, pred_g_mc_cov = nmcgpr.predict_g(test_x, full_cov=True)
    tf.debugging.assert_near(pred_g_analytical_cov, pred_g_mc_cov, rtol=0.01)
